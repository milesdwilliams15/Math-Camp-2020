...
library(ggridges)  # for plotting pretty distributions
library(foreach)   # for running loops
```
...
y = dnorm(x=x) # use defauls for mean and s.d.
...
y = pnorm(x) # use defauls for mean and s.d.
...
y = qnorm(x) # use defauls for mean and s.d.
...
x = rnorm(n=100) # use defauls for mean and s.d., and n = 100
...
You can download the .Rmd file that contains the problem set for this lesson from [here](http://github.com/milesdwilliams15/Math-Camp-2020/blob/master/distributions-in-R-problems.Rmd). You can see the questions in pdf format [here](http://github.com/milesdwilliams15/Math-Camp-2020/blob/master/distributions-in-R-problems.pdf). *Note*: For both links, right click to and select open in new tab.
